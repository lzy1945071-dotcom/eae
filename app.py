...
# Sidebar 切换布林带显示方式
bb_mode = st.sidebar.radio("布林带显示方式", ["叠加在主图", "单独副图"])
...
if bb_mode == "叠加在主图":
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_upper'], name="BB Upper",
                             line=dict(color="rgba(173,216,230,0.5)")), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_upper'], name="BB Upper",
                             line=dict(color="rgba(173,216,230,0.5)")), row=4, col=1)
    fig.add_trace(go.Scatter(x=df.index, y=df['BB_middle'], name="BB Middle",
...
if bb_mode == "叠加在主图":
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['RSI'], name="RSI",
                             line=dict(color="purple")), row=5, col=1)
