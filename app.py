...
volume_col = None
for cand_col in ["Volume", "volume", "vol", "Vol", "amt"]:
    if cand_col in dfi.columns:
        volume_col = cand_col
        break
if volume_col is None:
    dfi["_VolumeForHover"] = 0
...
fig.add_trace(
    go.Candlestick(
        ...
        close=dfi["Close"],
        name="K线",
        customdata=dfi[[volume_col]].values,
        hovertemplate=(
            "Time: %{x}<br>"
            + "Open: %{open}<br>"
            + "High: %{high}<br>"
            + "Low: %{low}<br>"
            + "Close: %{close}<br>"
            + "Volume: %{customdata[0]:,.2f}<extra></extra>"
        ),
    )
)
    ...
    for i, p in enumerate(parse_int_list(ma_periods_text)):
        ...
        if col in dfi.columns:
            fig.add_trace(go.Scatter(
                ...
                name=col,
                yaxis="y",
                line=dict(color=ma_colors[i % len(ma_colors, hoverinfo='skip')]),
        ...
        for i, p in enumerate(parse_int_list(ema_periods_text)):
            ...
            if col in dfi.columns:
                fig.add_trace(go.Scatter(
                    ...
                    name=col,
                    yaxis="y",
                    line=dict(color=ema_colors[i % len(ema_colors, hoverinfo='skip')]),
                    visible="legendonly"  # 默认隐藏
                ))
        ...
        for i, (col, nm) in enumerate([("BOLL_U","BOLL 上轨"),("BOLL_M","BOLL 中轨"),("BOLL_L","BOLL 下轨")]):
            if col in dfi.columns:
                fig.add_trace(go.Scatter(
                    ...
                    name=nm,
                    yaxis="y",
                    line=dict(color=boll_colors[i % len(boll_colors, hoverinfo='skip')]),
                    visible="legendonly"  # 默认隐藏
                ))
...
fig.add_trace(go.Scatter(
    ...
    mode="lines",
    name="支撑",
    line=dict(color="#00cc96", dash="dash", hoverinfo='skip'),
    yaxis="y",
    visible="legendonly"  # 默认隐藏
))
fig.add_trace(go.Scatter(
    ...
    mode="lines",
    name="阻力",
    line=dict(color="#ef553b", dash="dash", hoverinfo='skip'),
    yaxis="y",
    visible="legendonly"  # 默认隐藏
))
...
if not buy_signals.empty:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        name="买入信号",
        marker=dict(symbol="triangle-up", size=10, color="#00cc96", hoverinfo='skip'),
        visible="legendonly"  # 默认隐藏
    ))
...
if not sell_signals.empty:
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        name="卖出信号",
        marker=dict(symbol="triangle-down", size=10, color="#ef553b", hoverinfo='skip'),
        visible="legendonly"  # 默认隐藏
    ))
...
if use_macd and all(c in dfi.columns for c in ["MACD","MACD_signal","MACD_hist"]):
    fig.add_trace(go.Scatter(
        ...
        yaxis="y3",
        mode="lines",
        line=dict(color="#3366cc", hoverinfo='skip')
    ))
    fig.add_trace(go.Scatter(
        x=dfi.index,
        y=dfi["MACD_signal"],
        ...
        yaxis="y3",
        mode="lines",
        line=dict(color="#ff9900", hoverinfo='skip')
    ))
    fig.add_trace(go.Bar(
        x=dfi.index,
        y=dfi["MACD_hist"],
        ...
        yaxis="y3",
        opacity=0.4,
        marker_color=np.where(dfi["MACD_hist"] >= 0, "#00cc96", "#ef553b", hoverinfo='skip')
    ))
...
if use_rsi and "RSI" in dfi.columns:
    fig.add_trace(go.Scatter(
        ...
        yaxis="y4",
        mode="lines",
        line=dict(color="#17becf", hoverinfo='skip')
    ))
# 添加RSI超买超卖线
...
if use_kdj and all(c in dfi.columns for c in ["KDJ_K","KDJ_D","KDJ_J"]):
    fig.add_trace(go.Scatter(
        ...
        yaxis="y5",
        mode="lines",
        line=dict(color="#ff7f0e", hoverinfo='skip'),
        visible="legendonly"  # 默认隐藏
    ))
    fig.add_trace(go.Scatter(
        x=dfi.index,
    ...
    fig.add_trace(go.Scatter(
        ...
        yaxis="y5",
        mode="lines",
        line=dict(color="#2ca02c", hoverinfo='skip'),
        visible="legendonly"  # 默认隐藏
    ))
# 添加KDJ超买超卖线
...
fig_eq = go.Figure()
fig_eq.add_trace(go.Scatter(x=equity.index, y=equity.values, mode="lines", name="策略净值", hoverinfo='skip'))
...
if res is None:
    ...
    fig_c = go.Figure()
    fig_c.add_trace(go.Scatter(x=res["equity"].index, y=res["equity"].values, mode="lines", name="组合策略净值", hoverinfo='skip'))
