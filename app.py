...
# 1) 悬浮信息跟随鼠标（而非顶部统一框）
fig.update_layout(
    hovermode="closest",        # 以光标附近的点为准，提示框跟随鼠标
    hoverdistance=0,            # 光标必须贴点显示
    spikedistance=0             # 竖直/水平十字线紧贴鼠标
)
# 2) 所有坐标轴开启十字准星（spikes）并随光标吸附
for axis_name in [k for k in fig.layout if str(k).startswith(("xaxis","yaxis"))]:
    ax = fig.layout[axis_name]
    try:
        ax.showspikes = True
        ax.spikemode = "across"     # 垂直线贯穿所有子图（如有）
        ax.spikesnap = "cursor"     # 对齐到鼠标位置
        ax.spikecolor = "rgba(128,128,128,0.7)"
        ax.spikethickness = 1
    except Exception:
        pass

# 3) 优化 K 线的 hover 显示内容（在鼠标处展示详细数值）
fig.update_traces(
...
for axis_name in [k for k in fig_eq.layout if str(k).startswith(("xaxis","yaxis"))]:
    ax = fig_eq.layout[axis_name]
    try:
        ax.showspikes = True
        ax.spikemode = "across"
        ax.spikesnap = "cursor"
        ...
        pass
fig_eq.update_layout(hoverlabel=dict(namelength=-1))
# === 交互增强结束 ===
...
for axis_name in [k for k in fig_c.layout if str(k).startswith(("xaxis","yaxis"))]:
    ax = fig_c.layout[axis_name]
    try:
        ax.showspikes = True
        ax.spikemode = "across"
        ax.spikesnap = "cursor"
        ...
        pass
fig_c.update_layout(hoverlabel=dict(namelength=-1))
# === 交互增强结束 ===
